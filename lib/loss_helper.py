...
def compute_objectness_loss(data_dict):
    ...
    objectness_label = torch.zeros((B,K)).type_as(aggregated_vote_xyz).long()
    objectness_mask = torch.zeros((B,K)).type_as(aggregated_vote_xyz)
    ...
    criterion = nn.CrossEntropyLoss(torch.Tensor(OBJECTNESS_CLS_WEIGHTS).type_as(aggregated_vote_xyz), reduction='none')
...
def compute_box_and_sem_cls_loss(data_dict, config):
    ...
    heading_label_one_hot = torch.FloatTensor(batch_size, heading_class_label.shape[1], num_heading_bin).zero_().type_as(heading_class_label)
    ...
    size_label_one_hot = torch.FloatTensor(batch_size, size_class_label.shape[1], num_size_cluster).zero_().type_as(size_residual_label)
    ...
    mean_size_arr_expanded = torch.from_numpy(mean_size_arr.astype(np.float32)).unsqueeze(0).unsqueeze(0).type_as(size_residual_label) # (1,1,num_size_cluster,3) 
...
def compute_cap_loss(data_dict, max_len=CONF.TRAIN.MAX_DES_LEN+2):
    ...
    # unpack
    pred_caps = data_dict["lang_cap"] # (B*C, num_words - 1, num_vocabs)
    num_words = data_dict["lang_len"].reshape(-1).max()
    target_caps = data_dict["lang_ids"].reshape(-1, max_len)[:, 1:num_words] # (B*C, num_words - 1)
    ...
    # mask out bad boxes
    good_bbox_masks = data_dict["good_bbox_masks"].unsqueeze(1).repeat(1, num_words-1) # (B*C, num_words - 1)
    good_bbox_masks = good_bbox_masks.reshape(-1) # (B*C * num_words - 1)
    cap_loss = torch.sum(cap_loss * good_bbox_masks) / (torch.sum(good_bbox_masks) + 1e-8)
...
def get_loss(data_dict, config, 
    ...
    if detection:
        ...
        data_dict["vote_loss"] = torch.zeros(1)[0].type_as(vote_loss)
        data_dict["objectness_loss"] = torch.zeros(1)[0].type_as(vote_loss)
        data_dict["center_loss"] = torch.zeros(1)[0].type_as(vote_loss)
        data_dict["heading_cls_loss"] = torch.zeros(1)[0].type_as(vote_loss)
        data_dict["heading_reg_loss"] = torch.zeros(1)[0].type_as(vote_loss)
        data_dict["size_cls_loss"] = torch.zeros(1)[0].type_as(vote_loss)
        data_dict["size_reg_loss"] = torch.zeros(1)[0].type_as(vote_loss)
        data_dict["sem_cls_loss"] = torch.zeros(1)[0].type_as(vote_loss)
        data_dict["box_loss"] = torch.zeros(1)[0].type_as(vote_loss)
    ...
    if caption:
        ...
        # store
        data_dict["cap_loss"] = torch.zeros(1)[0].type_as(vote_loss)
        data_dict["cap_acc"] = torch.zeros(1)[0].type_as(vote_loss)
        data_dict["pred_ious"] =  torch.zeros(1)[0].type_as(vote_loss)
    ...
    if orientation:
        ...
        # store
        data_dict["ori_loss"] = torch.zeros(1)[0].type_as(vote_loss)
        data_dict["ori_acc"] = torch.zeros(1)[0].type_as(vote_loss)
    ...
    if distance:
        ...
        # store
        data_dict["dist_loss"] = torch.zeros(1)[0].type_as(vote_loss)
